...
class IntellijPlacer:
    ...
    def read_pics(self, path, pic_num):
        ...
        self.binarized_objects = [self.binarize(t) for t in self.pics]

        image = imageio.imread(path + "/small_res/" + str(pic_num) + ".jpg")
        self.img = self.binarize(image)

    def rotate(self, cnt, angle, img_size):         # поворот текстуры
        M = cv.moments(cnt)
        cx = int(M['m10'] / M['m00'])
        cy = int(M['m01'] / M['m00'])
    ...
    def get_pics(self, path, is_small=False):                # загрузка изображений
        if is_small and os.path.isdir(path + "/small_res"):
            pics = [imageio.imread(path + "/small_res/" + fname) for fname in os.listdir(path + "/small_res") if
                    fname.endswith(".jpg")]
            return
    ...
    def check_place(self, main_poly: np.ndarray):           # проверка расположений
        err_cnt = 0
        for el in main_poly.flat:
            if el > 255:
                err_cnt += 1
                if err_cnt > main_poly.shape[0] * main_poly.shape[1] / 5000:
                    return False
        return True
    ...
    def try_fit(self):                                                          # проверка на возможность располажение всех объектов
        ...
        for rec in rects:
            ...
            rotated_cnt = self.rotate(rotated_cnt, -rec[2], rec[1])
